...
for S in ${N[@]}; do
    ...
    mpirun -np 1 ./bin/lcs_parallel_io "$TMP_STR1" "$TMP_STR2" "$TMP_PAR_IO" "$TMP_P" "$TMP_S"
    md5sum "$TMP_P" "$TMP_S" > "$TMP_MD5"
    for i in ${PROC_COUNT[@]}; do
        mpirun -np $i ./bin/lcs_parallel_io "$TMP_STR1" "$TMP_STR2" "$TMP_PAR_IO" "$TMP_P" "$TMP_S"
        mpirun -np $i ./bin/lcs_parallel "$TMP_STR1" "$TMP_STR2" "$TMP_PAR"
        echo Number of processes = $i
        diff "$TMP_SEQ" "$TMP_PAR_IO"
        diff "$TMP_SEQ" "$TMP_PAR"
        md5sum -c --quiet "$TMP_MD5"
    done 
done
rm -f "$TMP_STR1" "$TMP_STR2" "$TMP_PAR_IO" "$TMP_PAR" "$TMP_P" "$TMP_S" "$TMP_SEQ" "$TMP_MD5"
